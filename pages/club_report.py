...
def _build_data_summary(
    ...
    if comparables_context:
        lines += ["MOST COMPARABLE PLAYERS (by statistical profile)", comparables_context, ""]
...
def _build_prompt(
    ...
    if comparables_context:
        lines.append(
            f"\nMOST COMPARABLE PLAYERS (statistically similar profile — use to signal "
            f"what profile clubs would lose if these players moved, and that {name} fills that gap):\n"
            f"{comparables_context}"
        )
...
if not _comparables_df.empty:
    ...
    st.caption(
        f"Players with the most similar statistical fingerprint to {selected_name} across "
        f"{ws_pos_key.lower()} peers. Clubs currently relying on these players represent "
        f"potential gaps your client could fill."
    )
    ...
    _comp_lines = []
    for _, _cr in _comparables_df.iterrows():
        _comp_lines.append(
            f"  {_cr['Player']} ({_cr.get('Team','?')}, {_cr.get('League','?')}) — "
            f"Similarity {_cr['Similarity']:.0f}%"
        )
    _comparables_context = "\n".join(_comp_lines)
...
_shared_args = dict(
    ...
    comparables_context=_comparables_context,
)
